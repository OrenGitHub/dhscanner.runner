...
|---:|---|---|---:|
| 1 | **HOC-unwrap clause** — recognize the *generic* HOC-shape entry-point pattern `export const <VERB> = async (req) => <ANY-CALLEE>(<dict-literal-with-callable-field>)` across **all HTTP verbs** `{GET, POST, PUT, PATCH, DELETE, HEAD, OPTIONS}`. Synthesize a call-graph edge from the top-level verb export to every callable value found inside the dict-literal argument. **Not a named catalog** (no hardcoded `authenticatedApiClient`); the outer HTTP-verb export scoping prevents random inner HOC shapes from firing. See [`OWASP26_NOTES.md`](OWASP26_NOTES.md) §14 Assignment 1 for the full design + heuristic legitimacy discussion. **Design change 2026-09-18:** originally scoped as an `authenticatedApiClient`-specific named-catalog fix; widened to a generic outer-syntax recognizer after design chat. **Auth classification is out of scope for this fix** — the endpoint stays labeled pre-auth even when the HOC enforces auth; that's a separate recognizer catalog (auth-enforcing HOC names) which can be added independently and is a *severity-mislabel only, conservative direction* imprecision. Byproduct of the verb-widening: `PUT /api/v2/management/contacts/bulk` (formbricks bulk contacts endpoint, currently absent from the POST-only snapshot) surfaces — this is the demo opening-act endpoint per [`OWASP26_NOTES.md`](OWASP26_NOTES.md) §5.11. | `utils.pl` — new generic recognizer clause (Prolog only; AST already preserves the shape, no parser changes) | 1/14 → 8/14 on auth-POST; also 0 → 1 on auth-PUT (surfaces `/contacts/bulk`) |
